...
(function(){
    ...
    function SearchController($scope, $http, SearchService, $rootScope, RestService, constant, PaginationService) {
        ...
        $scope.switchPage = function(index){
            if($scope.request){
                PaginationService.switchPage(index, constant.resourcesQuery + '/search?' + buildQuery($scope.request)+ '&').then(function(data){
                    $scope.resources = data.data;
                    $scope.currentPage = PaginationService.currentPage;
                });
            }else {
                PaginationService.switchPage(index, constant.resourcesQuery + '?').then(function(data){
                    $scope.resources = data.data;
                    $scope.currentPage = PaginationService.currentPage;
                });
            }
        ...
        $scope.cancelSearch = function(){
            $http.get('/rest.php/resources?&per-page=' + constant.perPage).then(function(data){
                $scope.resources = data.data;
                $scope.request = {};
            });
        };
